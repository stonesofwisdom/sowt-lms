...
export default function Auth() {
  ...
  async function submit() {
    ...
    try {
      if (mode === "signup") {
        ...
        setMsg("Account created! Your access is pending — drop your name and WhatsApp number on the group. Now sign in.");
        setMode("signin");
      } else if (mode === "forgot") {
  ...
  const cta = mode === "signup" ? "Sign up" : mode === "forgot" ? "Send reset link" : "Sign in";
      ...
      <div className="w-full max-w-md">
        ...
        <div className="rounded-3xl p-7" style={{ background: C.card, border: `1px solid ${C.line}` }}>
          ...
          <div className="mt-5 space-y-3">
            ...
            {mode !== "forgot" && <input value={pw} onChange={(e) => setPw(e.target.value)} type="password" placeholder="Password" onKeyDown={(e) => { if (e.key === "Enter") submit(); }} className="w-full rounded-2xl px-4 py-3 text-sm" style={inp} />}
          </div>
          <button onClick={submit} disabled={busy} className="btn mt-4 w-full rounded-2xl px-5 py-3 text-sm font-bold text-white inline-flex items-center justify-center gap-2" style={{ background: C.blue, opacity: busy ? 0.7 : 1 }}>{busy ? <Spinner light /> : null}{cta}</button>
          {msg && <div className="mt-3 text-xs" style={{ color: C.muted }}>{msg}</div>}
          {mode === "signin" && <div className="mt-3 text-center"><button onClick={() => { setMode("forgot"); setMsg(""); }} className="text-xs font-bold" style={{ color: C.muted }}>Forgot password?</button></div>}
          <div className="mt-4 text-center text-sm">
            {mode === "signup" && <>Already have an account? <button onClick={() => { setMode("signin"); setMsg(""); }} className="font-bold" style={{ color: C.blue }}>Sign in</button></>}
            {mode === "forgot" && <button onClick={() => { setMode("signin"); setMsg(""); }} className="font-bold" style={{ color: C.blue }}>Back to sign in</button>}
            {mode === "signin" && <>New here? <button onClick={() => { setMode("signup"); setMsg(""); }} className="font-bold" style={{ color: C.blue }}>Create an account</button></>}
          </div>
        </div>
      </div>
    </div>
